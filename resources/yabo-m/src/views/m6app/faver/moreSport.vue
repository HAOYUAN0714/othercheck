<template>
  <div class="dashboard-container">
    ...
    <div v-if="sortedEvent && sortedEvent.length > 0">
      ...
      <div v-if="collisionCheck.length > 0" class="combo" @click="openBet()">
        串
        <div class="tips">{{ collisionCheck.length }}</div>
      </div>
    <!-- <transition>
    <comboBet
...
export default {
  ...
  watch: {
    ...
    curBetSlip(val) {
      // this.curData = val
      // console.log(this.curData, 'ppppppppppppp')
      let finalInfo = []
      const collision = {}
      this.comboStatus = 0
      this.errorMsg = ''
  ...
  methods: {
    ...
    clickOdds(playId, betId, games, sportId, combo = 2) {
      // console.log(playId, betId, games, sportId,combo,'combo more')
      // console.log(this.curBetSlip,'123', this.getBetTypeData(games.MarketLines, playId, betId), 'this.getBetTypeData(games.MarketLines, playId, betId)')
      // console.log('bet', this.getBetTypeData(games.MarketLines, playId, betId))
      // console.log('playType', this.getMarketLineId(games.MarketLines, playId))
    ...
    getcollisionCheck(games) {
      // console.log(games, 'games')
      return (games.EventGroupId > 0) ? games.EventGroupId : games.EventId
    },
    // closeFuncAll(val) {
